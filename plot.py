...
if __name__ == '__main__':
    experiment_number_list = [10, 25, 50, 100]
    epochs = 19
    
    fig = plt.figure()
    for i_number in experiment_number_list:
        log_file_path = fr'D:\NCKU\Course\Data_Science\part3\HW4\facenet-pytorch-glint360k-master\logs\lfw_resnet34_train_{i_number}_log_triplet.txt'
        
        
        with open(log_file_path, 'r') as f:
            lines = f.readlines()
            epoch_list = [int(line.split('\t')[0]) for line in lines]
            accuracy_list = [round(float(line.split('\t')[1]), 4) for line in lines]
            
            plt.plot(epoch_list, accuracy_list, label=f'LFW Accuracy-training with {i_number}%')
            plt.ylim([0, 1])
            plt.xlim([0, epochs + 1])
            ...
            plt.title('LFW Accuracies plot')
            plt.legend(loc='lower right')
    fig.savefig('lfw_accuracies_comparison.png', dpi=fig.dpi)
    plt.close()
